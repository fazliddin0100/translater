...
st.caption(
    "PDF yoki Word yuklang → tarjima → natija doim Word (.docx) bo‘ladi (keyin tahrirlash oson)."
)
...
if uploaded is not None and run_btn:
    try:
        ...
        in_path.write_bytes(uploaded.getvalue())
        out_name = f"{Path(uploaded.name).stem}_uz.docx"
        ...
        status.success(f"Tayyor: {len(units)} bo‘lak tarjima qilindi → {out_name}")
    except Exception as exc:  # noqa: BLE001
        st.session_state.last_error = f"{exc}\n{traceback.format_exc()}"
...
if st.session_state.result_bytes:
        ...
        file_name=st.session_state.result_name or "tarjima_uz.docx",
